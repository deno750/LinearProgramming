...
int Simplex::solve(Matrix &mat) {
    ...
    while (!optimal) {
        ...
        bool hasInitialBasis = mat.basisSize() == sizeOfBasis;
        if (!hasInitialBasis) {
            ...
            unsigned artificialStartIndex = mat.getColumnsCount() - mat.getRowsCount() + 1;
            ...
            for (unsigned i = 1; i < mat.getRowsCount(); ++i) {
                for (unsigned j = artificialStartIndex; j < mat.getColumnsCount(); ++j) {
                    //std::cout << "i: " << i << " j: " << j - artificialStartIndex + 1 << std::endl;
                    if ((j - artificialStartIndex + 1) == i) {
                        mat[i][j] = Fraction::ONE;
                        mat.insertInBasis(i, j);
                    } else {
            ...
            if (bLessThan0 == 0) { //All b values are greater than 0. Optimal solution for dual is found. Also it is optimal for dual.
                if (appliesTwoPhasisMethod) {
                    ...
                    if (mat[0][0].getDoubleValue() == 0) {
                        ...
                        for (unsigned i = 1; i < mat.getRowsCount(); ++i) {
                            unsigned basisIndex = mat.getBasisIndexAtRow(i);
                            if (basisIndex > mat.getColumnsCount() - mat.getRowsCount()) { //Checking if an artificial variable is still in basis
                                
                                //Find the pivot
                                Fraction pivot;
                                Fraction mulForCosts;
                                for (unsigned j = 1; j < mat.getColumnsCount(); ++j) {
                                    if (mat[i][j].getDoubleValue() != 0 && j != basisIndex) {
                                        pivot = mat[i][j];
                                        mulForCosts = mat[0][j];
                                        
                                        std::cout << "The artificial variable with index " << basisIndex << " is still in basis.\nComputing the pivot operation.\nTaken pivot " << pivot.toString() << " at (" << i << ", " << j << ")\n" << std::endl;
                                ...
                                for (unsigned k = 0; k < mat.getRowsCount(); ++k) {
                                    ...
                                    unsigned basisIndex = mat.getBasisIndexAtRow(i);
                                    Fraction mul = mat[k][basisIndex]; //The value that is premoltiplicated to the pivot row in order to subtract the current row by the result
                                    for (unsigned j = 0; j < mat.getColumnsCount(); ++j) {
    ...
    if (optimal) {
        ...
        for (unsigned i = 1; i < mat.getRowsCount(); ++i) {
            cout << "x" << mat.getBasisIndexAtRow(i) << " = " << mat[i][0] << std::endl;
        }
        return 0;
    } else {
...
int Simplex::primalSolver(Matrix &mat) {
    ...
    mat.insertInBasis(pivotRowIndex, pivotColumnIndex);
...
unsigned Simplex::primalFindPivotRowIndex(Matrix &mat, unsigned pivotColumnIndex) {
    ...
    unsigned vectorInBasisIndexMin = mat.getColumnsCount() + 1; //Stores the minimum vector in basis for bland rule. Initialized with the maximum possible index
    for (unsigned i = 1; i < mat.getRowsCount(); ++i) {
        ...
        if (thetaTmp.getDoubleValue() <= thetaMin.getDoubleValue()) { //Checking if the theta canditate is the minimum theta.
            if (thetaTmp.getDoubleValue() == thetaMin.getDoubleValue()) { //For bland rule, exits in basis the vector with minimum index if the theta values are equals. Otherwise update the theta and minimum row index as usual
                //Bland rule
                if (mat.getBasisIndexAtRow(i) < vectorInBasisIndexMin) {
                    rowIndexMin = i;
                    thetaMin = thetaTmp;
...
unsigned Simplex::dualFindThetaRowIndex(Matrix &mat) {
    unsigned minimumRowIndex = 0;
    unsigned minimumVectorBasisIndex = mat.getColumnsCount() + 1; //Stores the minimum vector that exits from basis for the bland rule
    for (unsigned i = 1; i < mat.getRowsCount(); ++i) {
        unsigned vectorThatExitsFromBasisIndex = mat.getBasisIndexAtRow(i);
        if (mat[i][0].getDoubleValue() < 0) { //B value is < 0, so it should exit from basis
            if (vectorThatExitsFromBasisIndex < minimumVectorBasisIndex) { //Bland rule
                minimumRowIndex = i;
                minimumVectorBasisIndex = vectorThatExitsFromBasisIndex;
            }
        }
    }
    return minimumRowIndex; //Returns the index of the vector that exits the basis
}
